...
namespace YarnResearch.Common.UI
{
	...
	public readonly struct YarnIcon
	{
		...
		// One frame out of a multi-icon spritesheet, or null to draw the whole texture.
		...
		private YarnIcon(int itemType, string texturePath, Rectangle? frame, bool drawDropShadow)
		{
			_itemType = itemType;
		...
		public static YarnIcon FromItem(int itemType, bool drawDropShadow = true) =>
			new(itemType, null, null, drawDropShadow);

		public static YarnIcon FromTexture(string texturePath, Rectangle? frame = null, bool drawDropShadow = true) =>
			new(-1, texturePath, frame, drawDropShadow);
	...
	{
		public static readonly YarnIcon YarnCategory = YarnIcon.FromItem(ItemID.UnluckyYarn);
